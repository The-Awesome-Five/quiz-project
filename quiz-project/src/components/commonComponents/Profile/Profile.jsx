...
import { getUserDataByUID, updateUserSelectedItems } from "../../../services/user.service"; 
import './Profile.css';
...
const Profile = () => {
    ...
    const handleItemClick = async (item) => {
        if (selectedCategory === 'head') {
            ...
            await updateUserSelectedItems(userData.uid, { selectedHeadItem: item.image }); 
        } else if (selectedCategory === 'torso') {
            setSelectedTorsoItem(item.image);
            await updateUserSelectedItems(userData.uid, { selectedTorsoItem: item.image });
        } else if (selectedCategory === 'legs') {
            setSelectedLegsItem(item.image);
            await updateUserSelectedItems(userData.uid, { selectedLegsItem: item.image });
        }
        handleCloseModal(); 
    };
        ...
        <Container className="profile-page">
            <Row className="container">
                ...
                <Col xs={12} md={6} className="text-center text-md-start">
                    ...
                    <p className="mb-0">{profileData?.customInfo || 'No additional info provided.'}</p>
                </Col>
